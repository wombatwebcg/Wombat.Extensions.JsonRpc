...
namespace Wombat.Extensions.JsonRpc.Core.Client.Pool
{
    ...
    public class ConnectionPool : IConnectionPool
    {
        ...
        public async Task<IPooledConnection> CreateConnectionAsync(ConnectionEndpoint endpoint, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var connection = new PooledConnection(connectionId, endpoint, channel, 
                    _logger?.CreateLogger<PooledConnection>());
        ...
        private async Task<ITwoWayChannel> CreateChannelAsync(ConnectionEndpoint endpoint, CancellationToken cancellationToken)
        {
            ...
            var transport = transportFactory.CreateTransport(endpoint.TransportType);
            
            switch (endpoint.TransportType)
            {
                case TransportType.Tcp:
                    return await transport.ConnectAsync(endpoint.Address, endpoint.Port, cancellationToken);
                    
                case TransportType.WebSocket:
                    return await transport.ConnectAsync(endpoint.Address, endpoint.Port, cancellationToken);
                    
                case TransportType.NamedPipe:
                    return await transport.ConnectAsync(endpoint.Address, cancellationToken);
                    
                default:
                    throw new NotSupportedException($"不支持的传输类型: {endpoint.TransportType}");
            }
        }
