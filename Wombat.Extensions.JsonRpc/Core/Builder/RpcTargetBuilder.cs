    ...
    public class RpcTargetBuilder
    {
        ...
        public async Task<bool> RegisterServiceAsync(Type serviceType)
        {
            ...
            try
            {
                ...
                _logger?.LogInformation("成功注册RPC服务: {Service}, 方法数量: {Count}", 
                    serviceMetadata.ServiceName, serviceMetadata.Methods.Count);
        ...
        public void ApplyToJsonRpc(StreamJsonRpc.JsonRpc jsonRpc)
        {
            ...
            {
                jsonRpc.AddLocalRpcMethod(target.Key, target.Value);
            }
